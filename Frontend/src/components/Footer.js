    ...
    <footer className="bg-[#28509E] border-t border-white/10">
      <div className="max-w-[1200px] mx-auto px-[20px] md:px-[40px] pt-[28px] md:pt-[36px] pb-[0]">
        ...
        <div className="border-t border-white/10 pt-[20px] md:pt-[24px] pb-[8px]">
          ...
          <div className="border-t border-white/10 mt-[20px] md:mt-[24px]" />
          <div className="mt-[20px] md:mt-[24px] flex flex-col items-center gap-[12px]">
            {(() => {
              const secondaryMenu = t("footer.secondaryMenu", { returnObjects: true })
              const useSecondaryMenu = Array.isArray(secondaryMenu) && secondaryMenu.length > 0
              const linkClass = "font-['Open_Sans'] text-white/80 text-[13px] md:text-[14px] hover:text-[#FF8A00] transition-colors duration-200 underline focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-[#28509E] rounded-sm"
              if (useSecondaryMenu) {
                return (
                  <nav aria-label={t("footer.secondaryMenu.ariaLabel") || "Legal and info"} className="flex flex-wrap justify-center gap-x-[20px] gap-y-[8px]">
                    {secondaryMenu.map((item, idx) => (
                      <Link key={idx} to={langPath(item.path || "#")} className={linkClass}>
                        {item.label}
                      </Link>
                    ))}
                  </nav>
              ...
              return (
                <Link to={langPath("/privacy-policy")} className={linkClass}>
